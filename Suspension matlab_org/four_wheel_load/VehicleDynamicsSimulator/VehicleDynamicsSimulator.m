classdef VehicleDynamicsSimulator
    ...
    methods
        ...
        function obj = VehicleDynamicsSimulator(g_val)
            classInfo = dbstack('-completenames');
            classDir = fileparts(classInfo(1).file);
            dataFile = fullfile(classDir,'data.csv');
            if exist(dataFile, 'file')
                addpath(classDir);
                cfg = load_data(dataFile);
            else
                cfg = struct();
            end

            if nargin < 1
                obj.g = get_cfg(cfg, 'sim_g', 9.81);
            else
                obj.g = g_val;
            end
            
            % 車輛基本參數定義
            obj.m = get_cfg(cfg, 'sim_m', 360);
            obj.h = get_cfg(cfg, 'sim_h', 0.286);
            obj.W = obj.m * obj.g;
            obj.L_rear = get_cfg(cfg, 'sim_L_rear', 0.744);
            obj.L_front = get_cfg(cfg, 'sim_L_front', 0.806);
            obj.L = obj.L_front + obj.L_rear;
            
            obj.t_front = get_cfg(cfg, 'sim_t_front', 1.25);
            obj.t_rear = get_cfg(cfg, 'sim_t_rear', 1.25);
            
            obj.K = [get_cfg(cfg, 'sim_K_front_left', 30000), 0, 0, 0;
                     0, get_cfg(cfg, 'sim_K_front_right', 30000), 0, 0;
                     0, 0, get_cfg(cfg, 'sim_K_rear_left', 27000), 0;
                     0, 0, 0, get_cfg(cfg, 'sim_K_rear_right', 27000)];
                 
            obj.K_modal = [get_cfg(cfg, 'sim_K_modal_heave', 3050), 0, 0;
                           0, get_cfg(cfg, 'sim_K_modal_roll', 550), 0;
                           0, 0, get_cfg(cfg, 'sim_K_modal_pitch', 2500)];
                       
            coupled = get_cfg(cfg, 'sim_K_modal_coupled_roll_coupling', 2900);
            obj.K_modal_coupled = [get_cfg(cfg, 'sim_K_modal_coupled_heave', 3050), 0, 0, 0;
                                   0, get_cfg(cfg, 'sim_K_modal_coupled_roll_base', 600) + coupled, -coupled, 0;
                                   0, -coupled, get_cfg(cfg, 'sim_K_modal_coupled_pitch_base', 500) + coupled, 0;
                                   0, 0, 0, get_cfg(cfg, 'sim_K_modal_coupled_pitch', 2500)];
                               
            obj.K_tire = ones(1, 4) * get_cfg(cfg, 'sim_K_tire', 56000);
        end
...
function value = get_cfg(cfg, fieldName, defaultValue)
