%% 最小作用量原理求解靜不定系統
function  N = Four_wheel_load_LSM(ax,ay,F_add,CF_rela,car,check)
    ...
    % 慣性力相反 F + ma = 0 >> F = -ma
    b = car.m*[az ; ay*car.h ; ax*car.h];
    
    % 若有外力，加入外力作用
    if isfield(car,'F_add') && isfield(car,'CF_rela')
        r = CF_rela; % 外力作用點相對重心
        F = F_add;   % 外力向量 [Fx,Fy,Fz]
        % 外力對力矩的貢獻
    ...
    if check == true
        ...
        %平衡確認
        Fz = sum(N)+car.m*az+(isfield(car,'F_add')*F_add(3));
        fprintf('平衡確認 : Fz = %.2f N \n', Fz);

        % 力矩平衡檢查
        Mx = y_pos * N + car.m*ay*car.h;
        My = x_pos * N + car.m*ax*car.h;
...
% 空力
CF_rela = [0.0;0.0;0.0];  % 相對於重心(x,y,z)m
%}
